...
(() => {
  ...
  async function bootAudio() {
    ...
    try {
      ...
      // OPTIONAL master reverb
      const verb = Freeverb({ roomSize: .875, damping: .5 }).connect();
      kik.connect(verb); snare.connect(verb); hat.connect(verb);

      // Beat timing
      const beat = 22050;

      // Create bass synth with modulated cutoff
      window.bass = Synth({ Q: .85, gain: 0.15 })
        .connect(verb)
        .connect();

      bass.cutoff = Add(.45, Sine({ frequency: .125, gain: .4 }));

      // Create bass sequencer with base values
      window.bassSeq = Sequencer({
        target: bass,
        key: 'note',
